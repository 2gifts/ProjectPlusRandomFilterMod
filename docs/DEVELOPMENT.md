...
- **Hold Z while dropping on Random** — **Melee-style instant roll**: the
  character is rolled from your list right there and revealed on your panel.
  Pick the coin up (B) and Z+drop again to re-roll.
...
```
src/RANDSUB.ASM        the whole mod: 7 GCTRM hooks + storage (heavily commented)
...
2. `RSBE01.TXT` includes it (after CSSCustomControls) and has the old
   "Melee Random v2" hex block commented out (it was vestigial — that code never
   runs on P+'s CSS).
...
Seven hooks (all verified live against P+ 3.1.5; see `docs/addresses.md`):
...
2. **Drop hooks** (`$8068B818`/`$8068B828` and `$806892C4`/`$806892C8` — the
   game has two parallel random-drop code paths). Z held: roll from the list
   and place the rolled character (instant reveal). Z not held: keep slot `0x29`
   (mystery) and refresh the hidden pre-roll from the list. The clear gesture
...
4. **Legacy decide roll** (`HOOK @ $8068AE24`) — same logic on the vanilla
   decide path (replaces the old Melee Random v2; vestigial on P+'s CSS).
...
words @ `0x80002920`, drop choice @ `0x80002930`. Debug counters at
`0x80002908+`; strip before public release.
